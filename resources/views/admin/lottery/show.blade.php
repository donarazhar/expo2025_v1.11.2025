...
@section('title', 'Undian - ' . $event->judul)
@section('page-title', 'Undian')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-purple-700 rounded-xl shadow-lg p-8 text-white">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-3xl font-black mb-2">{{ $event->judul }}</h2>
                    <p class="text-purple-100 text-lg mb-2">{{ $event->formatted_date }}</p>
                    @if ($event->lokasi)
                ...
                <div class="text-right bg-white/10 backdrop-blur-sm rounded-lg p-4">
                    ...
                    <p class="text-5xl font-black">{{ $stats['total_eligible_participants'] }}</p>
                    <div class="flex items-center justify-end gap-4 mt-2 text-sm">
                        <div class="text-purple-100">
                            <span class="font-bold">{{ $stats['total_registered_attendees'] }}</span> Terdaftar
                        </div>
                        <span class="text-purple-300">•</span>
                        <div class="text-purple-100">
                            <span class="font-bold">{{ $stats['total_general_attendees'] }}</span> Umum
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Unik: {{ $stats['unique_winners'] }}</p>
                    </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <div class="flex items-start gap-3">
                ...
                <div class="text-sm text-blue-800">
                    <p class="font-semibold mb-1">Peserta Yang Eligible:</p>
                    <ul class="space-y-1">
                        <li>• <strong>Terdaftar:</strong> Peserta yang mendaftar event ini dan sudah check-in</li>
                        <li>• <strong>Umum:</strong> Peserta yang check-in tapi tidak terdaftar di event ini (eligible untuk
                            hadiah umum)</li>
                    </ul>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <!-- Prize Selection -->
                <div class="bg-white rounded-xl shadow-md p-6" x-data="lotteryMachine()">
                                ...
                                <div @click="selectPrize({{ json_encode($prize) }})"
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-gray-500">{{ $prize['event_name'] }}</p>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-600 font-semibold">Semua hadiah sudah habis</p>
                    ...
                    <!-- Lottery Machine -->
                    <div class="mt-6 bg-gradient-to-br from-purple-600 to-purple-800 rounded-xl shadow-2xl p-8">
                        <div class="text-center">
                            <h3 class="text-2xl font-black text-white mb-2">🎰 Mesin Undian</h3>
                            ...
                            <div
                                ...
                                <div x-show="winner" x-cloak class="text-center animate-fadeIn">
                                    <div class="mb-6">
                                        ...
                                        <div class="flex items-center justify-center gap-2">
                                            <span :class="winner?.type === 'registered' ? 'bg-blue-500' : 'bg-green-500'"
                                                class="text-white text-xs px-3 py-1 rounded-full font-bold"
                                                x-text="winner?.type === 'registered' ? '👤 Peserta Terdaftar' : '🌟 Peserta Umum'">
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="mt-4">
                                ...
                                <div x-show="selectedPrizeId && totalParticipants > 0"
                                    ...
                                    <p class="text-xs text-purple-200" x-show="breakdown">
                                        (<span x-text="breakdown?.registered || 0"></span> terdaftar +
                                        <span x-text="breakdown?.general || 0"></span> umum)
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-md p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('admin.lottery.export', $event) }}" target="_blank"
                            class="text-azhar-blue-500 hover:text-azhar-blue-600 text-sm font-semibold inline-flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                                    <div class="flex items-start justify-between gap-2 mb-2">
                                        <div class="flex-1 min-w-0">
                                            ...
                                            @if ($winner->registration && $winner->registration->peserta)
                                                <p class="text-xs text-gray-500 truncate">
                                                    {{ $winner->registration->peserta->asal_instansi ?? '-' }}</p>
                                            @endif
                                        </div>
                                        <div class="flex flex-col gap-1 items-end flex-shrink-0">
                                            ...
                                            @endif
                                            <span
                                                class="text-xs px-2 py-1 rounded-full {{ $winner->participant_type === 'registered' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }} font-semibold">
                                                {{ $winner->participant_type === 'registered' ? '👤 Terdaftar' : '🌟 Umum' }}
                                            </span>
                                        </div>
                                    </div>
    ...
    <script>
        // PERBAIKAN: Initialize claim modal as Alpine store
        ...
        function lotteryMachine() {
            return {
                ...
                async loadParticipants() {
                    try {
                        const response = await fetch(
                            `{{ route('admin.lottery.show', $event) }}/prize/${this.selectedPrizeId}/participants`);
                        const data = await response.json();
                ...
                async performDraw() {
                    ...
                    try {
                        const response = await fetch(
                            `{{ route('admin.lottery.show', $event) }}/prize/${this.selectedPrizeId}/draw`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                ...
                                body: JSON.stringify({
                                    ...
                                    registration_id: randomParticipant.id,
                                    type: randomParticipant.type
                                })
                            });
                        ...
                        if (data.success) {
                            ...
                            alert(data.message || 'Gagal melakukan undian');
                        }
                    } catch (error) {
                        clearInterval(this.rollingInterval);
                        this.isDrawing = false;
