...
@section('content')
    <div class="space-y-6">
        ...
        @if ($galleries->count() > 0)
            <!-- FIX: Gunakan satu instance Alpine.js untuk bulk actions dan sortable -->
            <div x-data="galleryManager()" x-init="initSortable()">

                <!-- Bulk Actions -->
                <div class="bg-white rounded-xl shadow-md p-4 mb-6">
                    <form action="{{ route('admin.galleries.bulk-delete') }}" method="POST"
                            ...
                            <div class="relative group" data-id="{{ $gallery->id }}">
                                ...
                                <div class="absolute top-2 left-2 z-10">
                                    <input type="checkbox" :checked="selectedIds.includes({{ $gallery->id }})"
                                        @change="toggleSelection({{ $gallery->id }})"
                                        class="w-5 h-5 text-azhar-blue-500 border-gray-300 rounded focus:ring-azhar-blue-500">
                                </div>
                                ...
                                <div class="mt-2">
                                    ...
                                    @if ($gallery->event)
                                        <p class="text-xs text-gray-500 mt-1 line-clamp-1">{{ $gallery->event->judul }}
                                        </p>
                                    @endif
                                </div>
            ...
            return {
                ...
                init() {
                    this.$watch('selectAll', value => {
                        if (value) {
                            this.selectedIds = {!! $galleries->pluck('id')->toJson() !!};
                        } else {
                            this.selectedIds = [];
                        }
                    });
                ...
                toggleSelection(id) {
                    ...
                    const totalItems = {!! $galleries->count() !!};
                    this.selectAll = this.selectedIds.length === totalItems;
                },
                            ...
                            try {
                                ...
                                if (data.success) {
                                    ...
                                    grid.querySelectorAll('[data-id]').forEach((el, index) => {
                                        const urutanEl = el.querySelector('.text-xs.text-gray-500');
                                        if (urutanEl) {
                                            urutanEl.textContent = '#' + (index + 1);
                                        }
                                    });
                                }
                            } catch (error) {
